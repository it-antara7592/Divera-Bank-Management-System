...
from ui.components.dialogs import ConfirmDialog, TransactionWithdrawalDialog, ManagerAuthorizationDialog

logger = logging.getLogger(__name__)


class WithdrawalPage(ctk.CTkFrame):
    """GUI page for processing account cash withdrawals."""
    ...
    def _create_header(self):
        ...
        subtitle = ctk.CTkLabel(
            header,
            text="Process secure account withdrawals and cash disbursals.",
    ...
    def _create_content(self):
        ...
        ctk.CTkLabel(
            card,
            text="Enter account details and specify withdrawal amount.",
        ...
        self._add_section_title(amount_card, "WITHDRAWAL AMOUNT")
    ...
    def search_account(self):
        ...
        if not account_number:
            tkmb.showwarning(
                ...
                "Please enter the account number for withdrawal."
            )
            return
    ...
    def handle_withdrawal(self):
        ...
        # 1. Validate through the TransactionService
        is_valid, errors, withdrawal_details = self.transaction_service.validate_withdrawal(
            account_number, amount_str
        )

        # 2. Handle failure (Insufficient balance, invalid account, or overdraft limit exceeded)
        if not is_valid:
            error_message = next(iter(errors.values())) if errors else "Invalid withdrawal details."
            
            # Log the failed transaction attempt in the database
            self.transaction_service.log_failed_withdrawal_attempt(
                account_number, amount_str, error_message
            )
            
            # Show error popup to the user
            tkmb.showerror("Withdrawal Failed", error_message)
            return

        # 3. Extract details if validation succeeds
        account = withdrawal_details["account"]
        amount = withdrawal_details["amount"]
        will_use_overdraft = withdrawal_details["will_use_overdraft"]
        ...
        # 5. Add an overdraft warning if dipping into credit
        if will_use_overdraft:
            msg += "\n⚠️ Warning: This withdrawal exceeds the available account balance and will utilize the approved overdraft facility."

        # Save pending state for the next confirmation step
        self.pending_withdrawal = withdrawal_details

        # 6. Open the structured confirmation dialog
        self._show_confirmation(msg)
        
    def _show_confirmation(self, message):
        try:
            TransactionWithdrawalDialog(
                self,
            ...
            logger.exception("TransactionWithdrawalDialog constructor mismatch.")
            tkmb.showerror("Dialog Error", "Unable to open the withdrawal confirmation dialog.")

    def _after_confirmation(self):
        try:
            if not self.pending_withdrawal:
                return
    ...
    def _after_authorization(self):
        ...
        withdrawal_details = self.pending_withdrawal
        self.pending_withdrawal = None

        success, message = self.transaction_service.execute_withdrawal(withdrawal_details)

        if success:
            tkmb.showinfo("Withdrawal Successful", message)
            self.clear_form()
        else:
            tkmb.showerror("Withdrawal Failed", message)
